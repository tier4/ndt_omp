  ...
  for(int64_t x_offset = -2; x_offset <= +2; x_offset++) {
    for(int64_t y_offset = -2; y_offset <= +2; y_offset++) {
      for(int64_t yaw_offset = 0; yaw_offset < 4; yaw_offset++) {
        ...
        timer.start();
        const int64_t limit_msec = 10000;
        ...
        // const initialpose_estimation::SearchResult result = initialpose_estimation::tpe_search(mg_ndt_omp, curr_initial_pose, limit_msec);
