#ifndef DISCRETE_TRANSFORMATION_HPP
#define DISCRETE_TRANSFORMATION_HPP
...
public:
  ...
  DiscreteTransformation(int score, int level, int x, int y, int z, int roll, int pitch, int yaw) : score(score), level(level), x(x), y(y), z(z), roll(roll), pitch(pitch), yaw(yaw) {}
          ...
          for(int l = 0; l < num_division.x(); l++) {
            for(int m = 0; m < num_division.y(); m++) {
              for(int n = 0; n < num_division.z(); n++) {
                b.emplace_back(DiscreteTransformation(0, child_level, x * v_rate + i, y * v_rate + j, z * v_rate + k, roll * num_division.x() + l, pitch * num_division.y() + m, yaw * num_division.z() + n));
              }
            }
          }
        }
...
#endif  // DISCRETE_TRANSFORMATION_HPP
