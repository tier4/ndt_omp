...
public:
  ...
  std::vector<Eigen::Vector3d> get_src_points() const {
    return src_points_;
  }
  ...
  std::vector<Eigen::Vector3d> get_angular_search_range() const {
    return std::vector<Eigen::Vector3d>{min_rpy_, max_rpy_};
  }

  Eigen::Matrix4d get_global_pose() const {
    return global_pose_;
  }
...
private:
  ...
  void calc_score(DiscreteTransformation<double>& trans, const double trans_res, const Eigen::Vector3d& rpy_res, const Eigen::Vector3d& min_rpy, const std::vector<Eigen::Vector4i>& buckets, const int max_bucket_scan_count, const std::vector<Eigen::Vector3d>& points);
