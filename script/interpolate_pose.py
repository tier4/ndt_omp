...
def interpolate_pose(
    ...
    * DataFrame with same columns as df_pose and length same as target_timestamp
    """
    POSITIONS_KEY = ["pose_x", "pose_y", "pose_z"]
    ORIENTATIONS_KEY = ["quat_w", "quat_x", "quat_y", "quat_z"]
    target_index = 0
    df_index = 0
    data_dict = {
        "timestamp": [],
    ...
    while df_index < len(df_pose) - 1 and target_index < len(target_timestamp):
        ...
        data_dict["pose_x"].append(target_position[0])
        data_dict["pose_y"].append(target_position[1])
        data_dict["pose_z"].append(target_position[2])
        data_dict["quat_w"].append(target_orientation[0])
        data_dict["quat_x"].append(target_orientation[1])
        data_dict["quat_y"].append(target_orientation[2])
        data_dict["quat_z"].append(target_orientation[3])
        target_index += 1
